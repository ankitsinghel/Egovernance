...
export async function POST(req: Request) {
  ...
  if (adminCount === 0) {
    ...
    if (role === "SuperAdmin") {
      return NextResponse.json(
        { ok: false, error: "SuperAdmin already exists" },
        { status: 403 }
      );
    }
  }
  ...
  const existing = await prisma.admin.findFirst({
    where: { departmentId: departmentId, stateId: stateId || null },
  });
  if (existing) {
    return NextResponse.json(
      {
        ok: false,
        error: "An admin for this department and city already exists",
      },
      { status: 409 }
    );
  }
